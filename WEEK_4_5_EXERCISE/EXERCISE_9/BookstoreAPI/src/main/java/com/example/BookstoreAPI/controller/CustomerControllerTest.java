...
public class CustomerControllerTest {
    ...
    public ResponseEntity<EntityModel<CustomerDTO>> createCustomerWithCustomHeaders(@Valid @RequestBody CustomerDTO customerDTO) {
        ...
        EntityModel<CustomerDTO> resource = EntityModel.of(createdCustomer,
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CustomerControllerTest.class).getCustomerById(createdCustomer.getId())).withSelfRel());
    ...
    public ResponseEntity<EntityModel<CustomerDTO>> getCustomerById(@PathVariable @Min(1) Long id) {
        ...
        EntityModel<CustomerDTO> resource = EntityModel.of(customer,
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CustomerControllerTest.class).getCustomerById(id)).withSelfRel(),
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CustomerControllerTest.class).getAllCustomers()).withRel("all-customers"));
    ...
    public ResponseEntity<CollectionModel<EntityModel<CustomerDTO>>> getAllCustomers() {
        List<EntityModel<CustomerDTO>> customers = customerService.getAllCustomers().stream()
                .map(customer -> EntityModel.of(customer,
                        WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CustomerControllerTest.class).getCustomerById(customer.getId())).withSelfRel()))
                .collect(Collectors.toList());

        // Add link to self
        CollectionModel<EntityModel<CustomerDTO>> collectionModel = CollectionModel.of(customers,
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CustomerControllerTest.class).getAllCustomers()).withSelfRel());
